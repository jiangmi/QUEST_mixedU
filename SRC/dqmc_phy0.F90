...
!--------------------------------------------------------------------!
  ...
  subroutine DQMC_Phy0_Print(P0, S, OPT)
    ...
    !call DQMC_Print_RealArray(0, nClass, &
    !     "Charge Density Wave correlation fn: (up-dn)", &
    !     S%clabel, P0%CDW(:, avg:avg), P0%CDW(:, err:err), OPT)
  ...
  subroutine DQMC_Phy0_Print1(P0, S, OPT1, OPT2, OPT3, OPT4, OPT5, OPT6)
    use dqmc_mpi
    !
    ! Purpose
    ! =======
    ...
    type(Phy0), intent(in)    :: P0   ! Phy0
    type(Struct), intent(in)  :: S    ! Underline lattice structure
    integer, intent(in)       :: OPT1, OPT2, OPT3, OPT4, OPT5, OPT6  ! Output file handle
    integer  :: i,k  
    ...
    ! Function terms
    call DQMC_Print_RealArray(0, nClass, "Mean Equal time Green's function:", &
         S%clabel, P0%G_fun(:, avg:avg), P0%G_fun(:, err:err), OPT1)

    call DQMC_Print_RealArray(0, nClass, "Up Equal time Green's function:", &
         S%clabel, P0%Gf_up(:, avg:avg), P0%Gf_up(:, err:err), OPT1)

    call DQMC_Print_RealArray(0, nClass, "Down Equal time Green's function:", &
         S%clabel, P0%Gf_dn(:, avg:avg), P0%Gf_dn(:, err:err), OPT1)

    ! Print the local density
    do i =1, P0%n
      k = S%D(i,i)
      write(OPT5,*) S%clabel(k), P0%Gf_up(k, avg:avg), P0%Gf_up(k, err:err)
      write(OPT6,*) S%clabel(k), P0%Gf_dn(k, avg:avg), P0%Gf_dn(k, err:err)
    enddo
...
!    call DQMC_Print_RealArray(0, nClass, &
!         "CDW correlation fn: ", &
!         S%clabel, P0%CDW(:, avg:avg), P0%CDW(:, err:err), OPT2)

    call DQMC_Print_RealArray(0, nClass, "XX Spin correlation function:", &
         S%clabel, P0%SpinXX(:, avg:avg), P0%SpinXX(:, err:err), OPT3)

    call DQMC_Print_RealArray(0, nClass, "ZZ Spin correlation function:", &
         S%clabel, P0%SpinZZ(:, avg:avg), P0%SpinZZ(:, err:err), OPT3)

    call DQMC_Print_RealArray(0, nClass, "Average Spin correlation function:", &
         S%clabel, P0%AveSpin(:, avg:avg), P0%AveSpin(:, err:err), OPT3)

    call DQMC_Print_RealArray(0, nClass, "Pairing correlation function:", &
         S%clabel, P0%Pair(:, avg:avg), P0%Pair(:, err:err), OPT4)

  end subroutine DQMC_Phy0_Print1
...
! modified code for PAM to obtain the total energy only involving
! c-f KE to get the entropy associated with heavy electrons
! Only need to add an additional if criterion
    ! loop all adj sites
    do i = 1, n  ! for each column
       do j = start(i), start(i+1)-1 ! for each nonzero elements
          if ( mod(abs(r(j)-i),2)==1) then
